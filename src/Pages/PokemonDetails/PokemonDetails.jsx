    ...
    const fetchEvolutionChain = async (speciesUrl) => {
        try {
            ...
            const extractEvolutionChain = async (chain) => {
                ...
                const traverseChain = async (currentChain, depth = 0) => {
                    if (!currentChain) return;

                    const id = currentChain.species.url.split('/').filter(Boolean).pop();
                    ...
                    for (const nextChain of currentChain.evolves_to) {
                        await traverseChain(nextChain, depth + 1);
                    }
                };
            ...
            console.error("Erro ao buscar a linha evolutiva:", error);
        }
    };
    ...
    useEffect(() => {
        ...
        const fetchPokemonData = async () => {
            try {
                ...
                fetchEvolutionChain(`${POKEMON_API_URL}-species/${id}`);
            } catch (error) {
            ...
            {pokemon ? (
                <>
                    <div className='details-image--container'>
                        ...
                        <img className='details-pokemon-image' src={`https://www.pokemon.com/static-assets/content-assets/cms2/img/pokedex/detail/${idUpdater(pokemon.id)}.png`} alt={pokemon.name} />
                    ...
                    <p className="evolution-title" style={{ margin: '1em' }}>Evolution Chain</p>
                    <div className="evolution-list">
                        <div className="evolution-list--container">
                            {[...new Set(evolutionChain.map((evo) => evo.depth))].map((depth) => (
                                <div key={depth} className='evolution-stage'>
                                    <div className="evolution-stage-items">
                                        {evolutionChain
                                            .filter((evo) => evo.depth === depth)
                                            .map((evo) => {
                                                return (
                                                    <div key={evo.id} className='evolution-card'>
                                                        ...
                                                        <div className="evolution-text-container">
                                                            ...
                                                            <p className='evolution-id'>Nº{threeZerosAfterId(evo.id)}</p>
                                                            <div className='evolution-types'>
                                                                {evo.types.map((type) => {
                                                                    return (
                                                                        <div key={type} className="evolution-type" style={{ backgroundColor: `var(--clr-${caseColor(type)})` }}>
                                                                            <div className="evolution-type-icon--container">
                                                                                <img src={caseTypeSmall(type)} alt={`${type} icon small`} />
                                                                            </div>
                                                                        </div>
                                                                    );
                                                                })}
